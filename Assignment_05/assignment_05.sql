...
-- 10
SELECT COUNT(*) AS nhan_vien FROM `Account`
...
-- 12
DELETE FROM `Exam`
WHERE CreateDate < '2019-12-20'; 		-- Không thể xóa do vi phạm ràng buộc khóa ngoại trong quá trình thực hiện thao tác.
...
-- 1 tạo view có chứa danh sách sinh viên thuộc "sale"
DROP VIEW IF EXISTS v_nhan_vien_sale;
CREATE VIEW v_nhan_vien_sale AS
SELECT * FROM `account`
WHERE DepartmentID = (
  SELECT DepartmentID FROM Department 
  WHERE DepartmentName = 'Sale'
...
SELECT * FROM v_account_tham_gia_nhieu_group;

-- 3 

-- 4 tao view phong ban co nhieu nhan vien nhat
DROP VIEW IF EXISTS v_phong_ban_nhieu_nhan_vien_nhat;
CREATE VIEW v_phong_ban_nhieu_nhan_vien_nhat AS
SELECT DepartmentID, DepartmentName, so_nhan_vien
FROM (
  SELECT d.DepartmentID, d.DepartmentName, COUNT(*) AS so_nhan_vien    -- NumEmployees = so_nhan_vien
  FROM Department d														-- DepartmentEmployeeCount = so_nhan_vien_phong_ban
  JOIN Account a ON d.DepartmentID = a.DepartmentID
  GROUP BY d.DepartmentID, d.DepartmentName
) AS so_nhan_vien_phong_ban
WHERE so_nhan_vien = (
  SELECT MAX(so_nhan_vien)
  FROM (
    SELECT d.DepartmentID, COUNT(*) AS so_nhan_vien
    FROM Department d
    JOIN Account a ON d.DepartmentID = a.DepartmentID
...
SELECT * FROM v_phong_ban_nhieu_nhan_vien_nhat;

-- 5 Tạo view có chứa tất các các câu hỏi do user họ Nguyễn tạo
DROP VIEW IF EXISTS v_cau_hoi_cua_nguyen ;
CREATE VIEW v_cau_hoi_cua_nguyen AS
SELECT q.QuestionID, q.Content, q.CategoryID, q.TypeID, q.CreatorID, q.CreateDate
FROM Question q
JOIN Account a ON q.CreatorID = a.AccountID
WHERE a.FullName LIKE 'Nguyễn%';


SELECT * FROM v_cau_hoi_cua_nguyen ;
